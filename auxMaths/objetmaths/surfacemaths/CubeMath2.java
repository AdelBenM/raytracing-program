...
//TODO    setBase(v.base());
  ...
  //========================
  public void reset(Point3 p, double c, Rn valeurRot, M3 baseRot) {
    centre=p;
    cote=c;
  //TODO    base = M3.tripleRotation(valeurRot, baseRot).fois(base);
    creerFaces();
  }
   ...
   * @param theta
   */
  public void faireTourner(Rn axe, double theta) {
	//TODO  base = M3.rotation(axe, theta).fois(base);
    creerFaces();
  }
   ...
   * @param baseRef
   */
  public void faireTourner(Rn valeurs, M3 baseRef) {
	//TODO  base= M3.tripleRotation(valeurs, baseRef).fois(base);
    creerFaces();
   ...
   * @param valeurs
   */
  public void faireTourner(Rn valeurs) {
	//TODO  base= M3.tripleRotation(valeurs, base).fois(base);
    creerFaces();
  ...
  public void creerFaces() {
    tabFaces[0]=new PlanMath(base.getC1(), centre.plus(base.getC1().prod(cote/2)));
    tabFaces[1]=new PlanMath(base.getC1().opp(), centre.plus(base.getC1().prod(-cote/2)));
    
    tabFaces[2]=new PlanMath(base.getC2(), centre.plus(base.getC2().prod(cote/2)));
    tabFaces[3]=new PlanMath(base.getC2().opp(), centre.plus(base.getC2().prod(-cote/2)));
    
    tabFaces[4]=new PlanMath(base.getC3(), centre.plus(base.getC3().prod(cote/2)));
    tabFaces[5]=new PlanMath(base.getC3().opp(), centre.plus(base.getC3().prod(-cote/2)));
